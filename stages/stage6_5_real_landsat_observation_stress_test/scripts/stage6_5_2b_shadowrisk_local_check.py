"""
Stage 6.5.2-B shadow-risk single-scene L2 SR local check + sealing.

This script:
- checks the B shadow-risk single-scene Landsat L2 SR package
- parses QA_PIXEL into a local valid mask
- aligns Stage 2 DEM / slope / aspect to the B sample grid
- computes cos_i with the real SUN_AZIMUTH / SUN_ELEVATION metadata
- compares near-zero / shadow ratios against the A clean high-sun baseline

It does not:
- download imagery
- run Stage 6.5.3 residual analysis
- use the old Stage 3 composite
...
B4_PATH = DATA_DIR / "stage6_5_shadowrisk_single_scene_l8_l2sr_b4_red.tif"
B5_PATH = DATA_DIR / "stage6_5_shadowrisk_single_scene_l8_l2sr_b5_nir.tif"
QA_PATH = DATA_DIR / "stage6_5_shadowrisk_single_scene_l8_l2sr_qa_pixel.tif"
METADATA_PATH = DATA_DIR / "stage6_5_shadowrisk_single_scene_l8_l2sr_metadata.csv"
...
DEM_ON_GRID_PATH = OUTPUT_DIR / "dem_on_shadowrisk_grid.tif"
SLOPE_ON_GRID_PATH = OUTPUT_DIR / "slope_on_shadowrisk_grid.tif"
ASPECT_ON_GRID_PATH = OUTPUT_DIR / "aspect_on_shadowrisk_grid.tif"
COS_I_PATH = OUTPUT_DIR / "cos_i_shadowrisk_grid.tif"
CONFIDENCE_PATH = OUTPUT_DIR / "confidence_shadowrisk_grid.tif"
QA_VALID_MASK_PATH = OUTPUT_DIR / "qa_valid_mask_shadowrisk_grid.tif"
NEAR_ZERO_MASK_PATH = OUTPUT_DIR / "near_zero_mask_shadowrisk_grid.tif"
SHADOW_MASK_PATH = OUTPUT_DIR / "shadow_mask_shadowrisk_grid.tif"
PREVIEW_PATH = OUTPUT_DIR / "stage6_5_2b_shadowrisk_local_check_preview.png"
REPORT_PATH = REPORT_DIR / "stage6_5_2b_shadowrisk_local_check_report.md"
...
CONFIDENCE_TAU = 0.1
CONFIDENCE_K = 30.0
A_NEAR_ZERO_RATIO = 0.000006
A_SHADOW_RATIO = 0.0
A_NEAR_ZERO_DESCRIPTION = "near_zero ratio ≈ 0.0006%"
A_SHADOW_DESCRIPTION = "shadow ratio = 0%"
TARGET_VALID_PIXEL_COVERAGE = 0.80
...
def percentile_stats_for_array(array, nodata=None, percentiles=(5, 50, 90, 95)):
    valid = valid_mask_for_array(array, nodata)
    if int(valid.sum()) == 0:
...
def write_report(context):
    ...
    text = f"""# Stage 6.5.2-B｜Shadow-risk Single-Scene 本地体检与封存
...
- Stage 6.5.1-B target VALID_PIXEL_COVERAGE: `{TARGET_VALID_PIXEL_COVERAGE:.2f}` ({TARGET_VALID_PIXEL_COVERAGE * 100:.1f}%)
...
- min: `{fmt(context['cos_stats']['min'])}`
- p5: `{fmt(context['cos_percentiles']['p5'])}`
- p50: `{fmt(context['cos_percentiles']['p50'])}`
- p95: `{fmt(context['cos_percentiles']['p95'])}`
- mean: `{fmt(context['cos_stats']['mean'])}`

### slope 统计

- mean slope: `{fmt(context['slope_stats']['mean'])}`
- p90 slope: `{fmt(context['slope_percentiles']['p90'])}`
...
- QA valid ratio: `{context['local_valid_ratio']:.6f}` ({context['local_valid_ratio'] * 100:.4f}%)
- near-zero ratio (`cos_i <= 0.1`): `{context['near_zero_ratio']:.6f}` ({context['near_zero_ratio'] * 100:.4f}%)
- shadow ratio (`cos_i <= 0`): `{context['shadow_ratio']:.6f}` ({context['shadow_ratio'] * 100:.4f}%)
- near-zero valid ratio (`QA valid 且 cos_i <= 0.1`): `{context['near_zero_valid_ratio']:.6f}` ({context['near_zero_valid_ratio'] * 100:.4f}%)
- shadow valid ratio (`QA valid 且 cos_i <= 0`): `{context['shadow_valid_ratio']:.6f}` ({context['shadow_valid_ratio'] * 100:.4f}%)
- safe ratio (`cos_i > 0.3`): `{context['safe_ratio']:.6f}` ({context['safe_ratio'] * 100:.4f}%)

## 与 A 样本对照

A 样本已封存为 clean high-confidence baseline sample：

- {A_NEAR_ZERO_DESCRIPTION}
- {A_SHADOW_DESCRIPTION}

B 样本判断：

- near-zero valid ratio 是否明显高于 A：**{context['near_zero_improved_status']}**
- shadow valid ratio 是否明显高于 A：**{context['shadow_improved_status']}**
- 综合判定：**{context['sample_decision']}**

解释：

{context['sample_decision_detail']}
...
def main():
    ...
    reflectance_analysis_valid = (
        local_valid
        & (b4["array"] >= REFLECTANCE_MIN)
        & (b4["array"] <= REFLECTANCE_MAX)
        & (b5["array"] >= REFLECTANCE_MIN)
        & (b5["array"] <= REFLECTANCE_MAX)
    )
    local_valid_ratio = ratio_from_bool(local_valid)
    analysis_valid_ratio = ratio_from_bool(reflectance_analysis_valid)
    ...
    near_zero[cos_i_valid] = (cos_i[cos_i_valid] <= CONFIDENCE_TAU).astype("uint8")
    ...
    cos_percentiles = percentile_stats_for_array(cos_i, FLOAT_NODATA, percentiles=(5, 50, 95))
    slope_stats = stats_for_array(slope_on_grid["array"], slope_on_grid["nodata"])
    slope_percentiles = percentile_stats_for_array(slope_on_grid["array"], slope_on_grid["nodata"], percentiles=(90,))

    cos_valid_count = int(cos_i_valid.sum())
    near_zero_bool = cos_i_valid & (cos_i <= CONFIDENCE_TAU)
    shadow_bool = cos_i_valid & (cos_i <= 0)
    safe_bool = cos_i_valid & (cos_i > 0.3)
    near_zero_ratio = int(near_zero_bool.sum()) / cos_valid_count if cos_valid_count else 0
    shadow_ratio = int(shadow_bool.sum()) / cos_valid_count if cos_valid_count else 0
    safe_ratio = int(safe_bool.sum()) / cos_valid_count if cos_valid_count else 0

    qa_cos_valid = local_valid & cos_i_valid
    qa_cos_valid_count = int(qa_cos_valid.sum())
    near_zero_valid_ratio = int((qa_cos_valid & (cos_i <= CONFIDENCE_TAU)).sum()) / qa_cos_valid_count if qa_cos_valid_count else 0
    shadow_valid_ratio = int((qa_cos_valid & (cos_i <= 0)).sum()) / qa_cos_valid_count if qa_cos_valid_count else 0

    near_zero_improved = near_zero_valid_ratio >= max(A_NEAR_ZERO_RATIO * 10, 0.001)
    shadow_improved = shadow_valid_ratio >= 0.001

    make_preview(b4, b5, qa_valid_out, dem_on_grid, slope_on_grid, cos_i, confidence, near_zero, shadow)

    basic_fail = (
        not all(grid_checks.values())
        or bool(metadata_missing)
        or not metadata.get("SUN_AZIMUTH")
        or not metadata.get("SUN_ELEVATION")
        or not (0 < local_valid_ratio <= 1)
        or not terrain_same_crs
        or not small_roi_within_dem
    )
    shadow_mechanism_ready = (near_zero_improved or shadow_improved) and not basic_fail

    if basic_fail:
        sample_decision = "FAIL"
        sample_decision_detail = "B 样本存在文件、网格、metadata、QA 或 DEM bounds 的硬失败项，不能进入 Stage 6.5.3-B。"
        next_step_status = "不建议进入 Stage 6.5.3-B；需要先修复 FAIL 项或重新导出 B 样本。"
    elif shadow_mechanism_ready:
        sample_decision = "PASS"
        sample_decision_detail = "B 样本文件健康，且 near-zero_valid_ratio 或 shadow_valid_ratio 明显高于 A clean high-sun baseline，可作为 shadow / near-zero mechanism test 候选。"
        next_step_status = "可以进入 Stage 6.5.3-B，但只做 shadow-risk single-scene 真实观测压力测试，不进入 Stage 7。"
    else:
        sample_decision = "WARNING"
        sample_decision_detail = "B 样本文件健康、地形栈可对齐，但 near-zero / shadow 危险域比例仍然很低；应封存为 low-sun partial sample，不直接进入背阴坡机制测试。"
        next_step_status = "不建议直接进入 Stage 6.5.3-B；先封存为 low-sun partial sample，必要时继续寻找更强 shadow-risk 候选。"
    ...
    gates = [
        ...
        },
        {
            ...
            "detail": f"Out-of-range reflectance ratios recorded; analysis_valid_ratio={analysis_valid_ratio:.6f}.",
        },
        {
        ...
        {
            "name": "B sample QA valid coverage should be interpreted cautiously",
            "status": "PASS" if metadata_valid_coverage >= TARGET_VALID_PIXEL_COVERAGE else "WARNING",
            "detail": (
                f"metadata VALID_PIXEL_COVERAGE={metadata_valid_coverage:.6f}; "
                f"target was {TARGET_VALID_PIXEL_COVERAGE:.2f}. Use QA/analysis masks downstream."
            ),
        },
        {
        ...
        {
            "name": "B sample should have clearly more near-zero / shadow danger than A baseline",
            "status": sample_decision,
            "detail": (
                f"near_zero_valid_ratio={near_zero_valid_ratio:.6f}, "
                f"shadow_valid_ratio={shadow_valid_ratio:.6f}, "
                f"A_near_zero_ratio≈{A_NEAR_ZERO_RATIO:.6f}, A_shadow_ratio={A_SHADOW_RATIO:.6f}"
            ),
        },
    ]
    ...
    context = {
        ...
        "analysis_valid_ratio": analysis_valid_ratio,
        ...
        "near_zero_valid_ratio": near_zero_valid_ratio,
        "shadow_valid_ratio": shadow_valid_ratio,
        "safe_ratio": safe_ratio,
        "near_zero_improved_status": "PASS" if near_zero_improved else "WARNING",
        "shadow_improved_status": "PASS" if shadow_improved else "WARNING",
        "sample_decision": sample_decision,
        "sample_decision_detail": sample_decision_detail,
        "gates": gates,
        "next_step_status": next_step_status,
    }
    write_report(context)

    print("Stage 6.5.2-B shadow-risk local check complete.")
