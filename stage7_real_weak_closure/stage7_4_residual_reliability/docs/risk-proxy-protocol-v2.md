# Stage 7.4｜Risk-proxy protocol v2

- **状态：** `frozen`；候选创建不产生冻结效力，唯一正式交叉审计轮全票 PASS 后，本协议与
  machine config 的完成字节才冻结并以 append-only 方式发布。
- **方法身份：** `A_geometry_margin_diagnostic / M2_guarded_5pct_v3`
- **冻结阶段：** 结果盲声明与 22 个上游 path/hash 已先于任何受保护结果读取冻结；protocol/config
  的内容冻结点为交叉审计通过之后、PF3 freeze evidence 写回之前。
- **冻结日期：** 2026-08-09（Asia/Shanghai 自然日；不主张日内时刻）
- **机器同伴：** `configs/risk-proxy-config-v2.json`
- **权威 preflight：** `evidence/preflight-manifest-v9.json`，SHA-256
  `b98f12cd2b5f9176c51b1955ff46e3e3040b8970140d81d8a09d4526e5ccf3bb`
- **结果盲声明：** `evidence/result-blind-declaration-v1.json`，SHA-256
  `b39dd6d585bd3142e9c4734efef002df33ddb99da40d765f063837cc4c9c783b`
...
本协议绑定 PF3 project `proj_6cbeea3266ff`、route `rt_f634f0ab72fb`。候选编译前的 live
readback 为 route revision `71`，Stage 7.3 `nd_38d128c40d90=done`，Stage 7.4
`nd_095e71a9eccf=active`。与本恢复周期直接相关的稳定事件链为：

1. Stage 7.4 激活 `rc_b9bb7bb94549`：`68→69`；
2. owner R1 决策 `rc_a7fb4eb449b6`：`69→70`；
3. 结果盲声明写回 `rc_4561f3d19c6c`：`70→71`。

上述 receipts 均已读回且 `undoneBy=null`；没有 rollback 或 reactivation。项目规则
`pr_d4a37fa568df` 已由 `rc_b40e7b5ee356` 修订到 revision 2：首动作只冻结“尚未读取结果”
声明与上游 path/hash；protocol/config 候选可在结果盲条件下编译和审计；其完成字节仅在唯一正式
交叉审计轮全票 PASS 后冻结，并须在首次结果读取前写入 PF3、回读真实 receipt、active state 与
live revision。

历史 v1 pair 的首个 patch 在 revision 2 下不构成有效内容冻结；其唯一保留效力是结果盲声明与
上游 hash 快照。v1 pair 与 audit-v1 均只作不可变事故历史，不回滚、不重开节点，也不产生新的
方法或输出版本。
...
数值相等但 bit pattern 不同的抽象分支按 R1 保留为 fail-closed 闭集成员：
`status=protocol_error`，`reason=numeric_equal_bit_distinct_proxy_order_undefined`。对本协议
唯一 proxy `risk_proxy=1-cos_i`，该分支对有限输入不可达：相等减法产生 `+0.0`，NaN 不满足
数值相等。本协议不为该不可达分支增加检测机制、次级排序或任何产物。
...
- `T=0`：`status=protocol_error`，`reason=zero_core_total_pixel_count`，且 `curve_shape=null`、`risk_proxy_verdict=null`；
...
新增默认、fallback、阈值、score、grid、积分、verdict 或 output category。

候选创建不冻结内容。唯一正式交叉审计轮必须同时覆盖 v9、完成的 v2 pair、executor 与 tests；
只有全票 PASS 才冻结该轮所审的 exact bytes 并允许 append-only 发布。随后必须把 v9、v2 pair、
audit、executor 与 tests 的 hash 写入 PF3，并以 fresh readback 证明 Stage 7.4 仍 active、receipt
未撤销且 live revision 已前进；本地 executor 还须绑定该 audit 的 hash，并由操作者传入该次真实
PF3 freeze receipt/revision。完成这次 PF3 回读前，仍禁止读取受保护结果或执行实验。

审计不全票通过或 PF3 写回／回读失败时直接 fail closed：不读取结果、不执行实验、不出 v10、
protocol-v3 或 config-v3。通过且回读后直接执行 Stage 7.4；六个既定 v1 输出路径保持 append-only
且禁止覆盖。
